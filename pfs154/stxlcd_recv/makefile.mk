...
ifeq ($(PROGRAMMER),1)
	$(TOOLPATH)/easypdkprog -n $(MCU) write $(PROJECT).ihx 1>&2
endif

ifeq ($(PROGRAMMER),2)

	$(TOOLPATH)/pfsprog txwr $(SERPORT) $(PROJECT).ihx $(NOWAIT) 1>&2
	$(TOOLPATH)/pfsprog run $(SERPORT) $(NOWAIT) 1>&2

endif

ifeq ($(PROGRAMMER),3)

	$(TOOLPATH)/pfsprog wr $(SERPORT) $(PROJECT).ihx $(NOWAIT) 1>&2
	$(TOOLPATH)/pfsprog run $(SERPORT) $(NOWAIT) 1>&2
...
ifeq ($(PROGRAMMER),2)
	...
	$(TOOLPATH)/pfsprog stop $(SERPORT) $(NOWAIT) 1>&2
