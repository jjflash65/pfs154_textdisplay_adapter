...
uint8_t wherex, wherey;
  ...
  #define uart_delay    106    // 106 us entsprechen ca. 9600 Bd

  /* --------------------------------------------------
                       uartsw_putchar

      Zeichen mit Software-UART senden
     -------------------------------------------------- */
  void uartsw_putchar(uint8_t ch)
  {
    uint8_t i;

    txd_clr();                    // Startbit
    delay_us(uart_delay);         // entspricht ca. 9600 Bd

    for (char i= 0; i< 8; i++)
    {
      if (ch & 0x01) txd_set(); else txd_clr();
      delay_us(uart_delay);
      ch >>= 1;
    }
    txd_set();
